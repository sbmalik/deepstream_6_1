...
TEST_IMG_PATH = f"{BASE_PATH}/data/known_faces_seq/img05.jpg"
...
def plot_one_box(x, landmark,img, color=None, label=None, line_thickness=None):
    ...
    h, w, c = img.shape
    left_eye = [landmark[0]]
    right_eye = [landmark[2]]
    print(left_eye)
    print(right_eye)
    print(math.dist(left_eye, right_eye))

    cv2.circle(img, (int(landmark[0]), int(landmark[1])), 1, (0, 0, 255), 4)
    cv2.circle(img, (int(landmark[2]), int(landmark[3])), 1, (0, 0, 255), 4)
...
if __name__ == "__main__":
    ...
    PLUGIN_LIBRARY = f"{BASE_PATH}/models/retinaface/libRetinafaceDecoder.so"
    ctypes.CDLL(PLUGIN_LIBRARY)
    engine_file_path = f"{BASE_PATH}/models/retinaface/retina_r50.engine"

    retinaface = Retinaface_trt(engine_file_path)
    input_image_paths = [TEST_IMG_PATH]
    retinaface.infer(input_image_paths[0])
    # for i in range(10):
